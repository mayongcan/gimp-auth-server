	...
	public JSONObject checkLoginIp(HttpServletRequest request, @RequestBody Map<String, Object> params) {
		...
		try {
			...
			if (userInfo == null)
				...
				if (isInList) {
					// 判断用户是否使用了不通的IP登录
					if (StringUtils.isNotBlank(userLogon.getLastLogonIp())
							&& !userLogon.getLastLogonIp().equals(ipAddr)) {
						logger.warn("用户:" + userInfo.getUserCode() + "上次登录IP:" + userLogon.getLastLogonIp() + ",当前登录IP:"
								+ ipAddr);
					}
					// 写入登录信息
