...
public class WebSecurityConfiguration extends WebSecurityConfigurerAdapter {
    ...
    public void configureGlobal(AuthenticationManagerBuilder auth) throws Exception {
        auth.authenticationProvider(userAuthenticationProvider); // 自定义验证
    ...
    /**
     * 添加跨域过滤器，需要在spring security验证前添加到过滤器，否则无法正常获取登录token 注意：在这里添加后，就不需要在子服务里面在添加，否则会出现重复响应头的问题
     * @return
     */
    @Bean
    ...
    protected void configure(HttpSecurity httpSecurity) throws Exception {
        ...
        httpSecurity.authorizeRequests() // 配置安全策略
                .antMatchers(HttpMethod.OPTIONS, "/oauth/token").permitAll() // 定义 请求不需要验证
